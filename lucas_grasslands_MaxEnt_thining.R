...
occs_all_releve_1_sf
occs_all_releve_1_sf_coords <- data.table(st_coordinates(occs_all_releve_1_sf))
occs_all_releve_1_sf_1 <- cbind(data.table(occs_all_releve_1_sf), occs_all_releve_1_sf_coords)
occs_all_releve_1_sf_1
...
taxons <- c("Cerastium fontanum")
taxons <- c("Rumex obtusifolius")

occs_all <- occs_all_releve_1_laea;   length(unique(occs_all$species))
occs_all <- occs_all_releve_1_sf_1;   length(unique(occs_all$species))
reps <- 1:5
thinning_distance <- c(2, 3, 5, 10)
thinning_distance <- c(1, 2, 3, 5, 10)




for(th in thinning_distance){
  #th <- thinning_distance[1]
  print(paste0("running thinning distance... ", th))
  ...
  for(r in reps){
    ...
    for (t in taxons){
      ...
      occs_i[, species := gsub(" ", "_", sps)]
      
      #t00 <- Sys.time()
      thinned <- thin(loc.data = occs_i,
                      lat.col = "Y",
                      long.col = "X",
                      spec.col = "species", 
                      thin.par = th,
      ...
      #Sys.time() - t00
      #
      thin_vals <- summaryThin(thinned)
      thin_vals <- which(thin_vals$Number_of_Data_frames_per_number_of_records[2, ] == thin_vals$Number_of_data_frames_with_maximun_number_of_records)[1]
      thin_vals
      #str(thinned)
      #head(thinned[[2]])
      #
      #identical(sort(thinned[[2]]$Latitude), sort(thinned[[1]]$Latitude))
      #identical(sort(thinned[[2]]$Longitude), sort(thinned[[1]]$Longitude))
      #
      #dev.off()
      #plotThin(thinned)
      
      #thinned_data <- fread("thinned_data_thin3.csv", header = TRUE)
      
      
      
      #occs_i <- data.table(thinned_data)
      occs_i <- data.table(thinned[[thin_vals]])
      setnames(occs_i, c("Longitude", "Latitude"), c("X", "Y"), skip_absent = TRUE)
      
      
      ##
      
      occurrences_raw_thin <- nrow(occs_i)
      ...
      occs_i_shp <- st_transform(st_as_sf(occs_i_shp), crs = st_crs(all_vars))
      
      #names(occs_i_shp) <- t
      #st_write(occs_i_shp,  "occs_i_shp_Thinned_5_wgs84_kk.shp", append = FALSE)
      ...
      data2save <- data.frame(species = t, 
                              thinning_dist = th,  
                              occurrences_raw,
                              occurrences_raw_thin,
                              occurrences_1km, 

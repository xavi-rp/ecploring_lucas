...
### Modelling with RF ####
# see doi: 10.1111/ecog.05615
# and https://onlinelibrary.wiley.com/action/downloadSupplement?doi=10.1111%2Fecog.05615&file=ecog12810-sup-0001-AppendixS1.pdf
...
for(r in reps){
  ...
  for (t in taxons){
    ...
    dir2save_rf <- paste0("models_rf_", t, "/")
    if(!dir.exists(paste0("models_rf_", t))) {
      dir.create(dir2save_rf)
    }
    ...
    sps_data_absences_train <- sample_n(sps_data_absences, ceiling(nrow(sps_data_absences) * prop4train)) # subset of background points for testing
    #                                                                                                     # not sure this is needed/correct 
    sps_data_absences_test <- sps_data_absences[!sps_data_absences$cells %in% sps_data_absences_train$cells, ]
    ...
    write.csv(sps_data_absences_train, paste0(dir2save_presences, "/sps_data_absences_train_", t, "_", r, ".csv"), row.names = FALSE)
    write.csv(sps_data_absences_test, paste0(dir2save_presences, "/sps_data_absences_test_", t, "_", r, ".csv"), row.names = FALSE)
    
    #sps_data_presences_train <- fread(paste0(dir2save_presences, "/sps_data_presences_train_", t, "_", r, ".csv"), header = TRUE)
    #sps_data_presences_test <- fread(paste0(dir2save_presences, "/sps_data_presences_test_", t, ".csv"), header = TRUE)
    #sps_data_absences_train <- fread(paste0(dir2save_presences, "/sps_data_absences_train_", t, ".csv"), header = TRUE)  
    #sps_data_absences_test <- fread(paste0(dir2save_presences, "/sps_data_absences_test_", t, ".csv"), header = TRUE)  
    
    
    library(randomForest)
    library(tidyr)
    
    sps_data_presences_train <- sps_data_presences_train[, .SD, .SDcols = names(all_vars)]
    sps_data_presences_train[, pres := 1]
    sps_data_absences_train <- sps_data_absences_train[, .SD, .SDcols = names(all_vars)]
    sps_data_absences_train[, pres := 0]
    sps_data_presences_test <- sps_data_presences_test[, .SD, .SDcols = names(all_vars)]
    sps_data_presences_test[, pres := 1]
    sps_data_absences_test <- sps_data_absences_test[, .SD, .SDcols = names(all_vars)]
    sps_data_absences_test[, pres := 0]
    
    
    data4training <- rbind(sps_data_presences_train, sps_data_absences_train)
    data4training
    data4testing <- rbind(sps_data_presences_test, sps_data_absences_test)  # Not sure that I can use same background points for testing
    data4testing
    
    data4training$pres <- as.factor(data4training$pres) # if this is factor, RF makes classifications
    #data4training$pres <- as.numeric(as.character(data4training$pres)) # if this is numeric, RF makes regression
    data4testing$pres <- factor(data4testing$pres, levels = c(0, 1)) 

    
    NumPres <- sum(data4training$pres == 1)         # number of presences
    sample_size <- c("0" = NumPres, "1" = NumPres)  # sample size for both classes
    
    # RF with down-sampling
    t0 <- Sys.time()
    rf_DownSampling <- randomForest(pres ~ .,
                                    data = data4training,
                                    #xtest = data4testing[, .SD, .SDcols = names(all_vars)],  # for testing (conf matrix)
                                    #ytest = data4testing$pres,                               # for testing (conf matrix)
                                    keep.forest = TRUE,
                                    ntree = 1000,
                                    sampsize = sample_size,
                                    replace = TRUE) # samples with replacement (default)
    
    Sys.time() - t0
    
    
    rf_DownSampling
    rf_DownSampling$confusion
    #rf_DownSampling$test$confusion
    rf_DownSampling$type
    head(rf_DownSampling$err.rate)
    rf_DownSampling$ntree
    
        
    save(rf_DownSampling, file = paste0(dir2save_rf, "models_", t, ".RData"))
    occurrences_train <- sum(sps_data_presences_train$pres)
    occurrences_test <- sum(sps_data_presences_test$pres)  # none if cross-validation
    background_points <- length(rf_DownSampling$y) - occurrences_train
    ...
    sps_predictions_rf_DownSampling <- predict(object = rf_DownSampling,
                                               newdata = all_vars_data, 
                                               type = "prob")#, 
                                               #type = "response")#, 
                                               #index = 2)
    
    sps_predictions_rf_DownSampling
    head(sps_predictions_rf_DownSampling)
    head(sps_predictions_rf_DownSampling_r)
    nrow(sps_predictions_rf_DownSampling)
    head(sps_predictions_rf_DownSampling[, 2])
    range(sps_predictions_rf_DownSampling[, 2])
    summary(sps_predictions_rf_DownSampling[, 2])
    ...
    all_vars_data1[, predictions := as.vector(sps_predictions_rf_DownSampling[,"1"])]
    ...
    names(sps_preds_rstr) <- "predictions_RF_DownSampling"
    
    #rm(worldclim_all_data0); gc()
    
    
    #pdf("sps_predictions_RF_DS_kk.pdf", width = 20, height = 15)
    #par(mfrow = c(1, 2))
    ...
    sps_data_presences_train_coords <- fread(paste0(dir2save_presences, "/sps_data_presences_train_", t, "_", r, ".csv"), header = TRUE)
    sps_data_presences_train_coords <- xyFromCell(object = occs_i_rstr,
                                                  cell = sps_data_presences_train_coords$raster_position, 
                                                  spatial = FALSE)
    ...
    sps_data_absences_train_coords <- fread(paste0(dir2save_presences, "/sps_data_absences_train_", t, "_", r, ".csv"), header = TRUE)  
    sps_data_absences_train_coords <- xyFromCell(object = occs_i_rstr,
                                                 cell = sps_data_absences_train_coords$cells, 
                                                 spatial = FALSE)

        
    train_preds <- data.frame(preds = raster::extract(sps_preds_rstr, sps_data_presences_train_coords))
    train_preds$pres <- 1
    
    train_preds_1 <- data.frame(preds = raster::extract(sps_preds_rstr, sps_data_absences_train_coords))
    train_preds_1$pres <- 0
    
    train_preds <- rbind(train_preds, train_preds_1)
    
    roc_rf <- AUC::roc(train_preds$preds, as.factor(train_preds$pres))
    AUC_rf <- round(AUC::auc(roc_rf), 3)
    AUC_rf
    
 

        
    sps_data_presences_test_coords <- fread(paste0(dir2save_presences, "/sps_data_presences_test_", t, "_", r, ".csv"), header = TRUE)
    sps_data_presences_test_coords <- xyFromCell(object = occs_i_rstr,
                                                 cell = sps_data_presences_test_coords$raster_position, 
                                                 spatial = FALSE)
    ...
    sps_data_absences_test_coords <- fread(paste0(dir2save_presences, "/sps_data_absences_test_", t, "_", r, ".csv"), header = TRUE)  
    sps_data_absences_test_coords <- xyFromCell(object = occs_i_rstr,
                                                 cell = sps_data_absences_test_coords$cells, 
                                                 spatial = FALSE)
    
    
    test_preds <- data.frame(preds = raster::extract(sps_preds_rstr, sps_data_presences_test_coords))
    test_preds$pres <- 1
    
    test_preds_1 <- data.frame(preds = raster::extract(sps_preds_rstr, sps_data_absences_test_coords))
    test_preds_1$pres <- 0
    
    test_preds <- rbind(test_preds, test_preds_1)
    
    roc_rf_val <- AUC::roc(test_preds$preds, as.factor(test_preds$pres))
    AUC_rf_val <- round(AUC::auc(roc_rf_val), 3)
    AUC_rf_val
    ...
    thresholds <- dismo::threshold(dismo::evaluate(raster::extract(sps_preds_rstr, raster::xyFromCell(occs_i_rstr, which(occs_i_rstr[] == 1))), 
                                                   raster::extract(sps_preds_rstr, bckgr))) # sensitibity default 0.9
    
    thresholds$sensitivity.99 <- dismo::threshold(dismo::evaluate(raster::extract(sps_preds_rstr, raster::xyFromCell(occs_i_rstr, which(occs_i_rstr[] == 1))),
                                                                  raster::extract(sps_preds_rstr, bckgr)),
                                                  stat = "sensitivity", sensitivity = 0.99) 
    
    if(any(thresholds < 0)) thresholds[thresholds < 0] <- 0
    ...
    sps_preds_rstr_pres_abs <- reclassify(sps_preds_rstr[["predictions_RF_DownSampling"]], rcl = thr, filename = '', include.lowest = FALSE, right = TRUE)
    sps_preds_rstr_pres_abs_all <- brick(sps_preds_rstr_pres_abs)
    names(sps_preds_rstr_pres_abs_all) <- c("Pres_Abs_RF_DownSampling")
    
    #plot(sps_preds_rstr_pres_abs)
    

    png(paste0(dir2save_rf, "sps_predictions_RF_DownSampling_", t, "_", r, ".png"), width = 20, height = 25, units = "cm", res = 75)
    ...
    plot(sps_preds_rstr[["predictions_RF_DownSampling"]], zlim = c(0, 1), main = "RF (DownSampling) predictions", cex.main = 1.2, cex.sub = 1.5)
    ...
    names(points4eval) <- c("presence", "RF_DS_preds")
    points4eval$ID_fake <- 1:nrow(points4eval)
    points4eval <- points4eval[, c("ID_fake", "presence", "RF_DS_preds")]
    points4eval <- points4eval[!is.na(points4eval$RF_DS_preds), ]
    head(points4eval)
    ...
    releve_point_sp_4modelling_laea_i$RF_DownSampling_preds <- raster::extract(sps_preds_rstr[["predictions_RF_DownSampling"]], 
                                                                               releve_point_sp_4modelling_laea_i[, .SD, .SDcols = c("X", "Y")]
    )
    
    releve_point_sp_4modelling_laea_i
    sum(is.na(releve_point_sp_4modelling_laea_i$RF_DownSampling_preds))  # 20 points with no prediction
    
    
    #obs_pred_conf <- cmx(releve_point_sp_4modelling_laea_i[, .SD, .SDcols = c("POINT_ID", "presence", "RF_DownSampling_preds")], 
    ...
    for(tr in (1:length(thresholds))){
      ...
      obs_pred_conf_i <- cmx(releve_point_sp_4modelling_laea_i[, .SD, .SDcols = c("POINT_ID", "presence", "RF_DownSampling_preds")], 
                             threshold = as.numeric(thresholds[tr]),
                             na.rm = TRUE)
      #print(obs_pred_conf_i)
    ...
    write.csv(data2save_ReleveValid, "info_models_RF_DS_ReleveValid.csv", row.names = FALSE)
    write.csv(data2save_ReleveValid, paste0(dir2save_rf, "info_models_RF_DS_ReleveValid.csv"), row.names = FALSE)
    ...
    roc <- AUC::roc(preds_clean$RF_DownSampling_preds, as.factor(preds_clean$presence))
    
    png(paste0(dir2save_rf, "RF_DS_", "roc_", t, ".png"), width = 15, height = 15, units = "cm", res = 75)
    ...
    write.csv(data2save_ReleveValid_AUC, "info_models_RF_DS_ReleveValid_AUC.csv", row.names = FALSE)
    write.csv(data2save_ReleveValid_AUC, paste0(dir2save_rf, "info_models_RF_Ds_ReleveValid_AUC.csv"), row.names = FALSE)
    ...
    data2save <- data.frame(species = t, occurrences_raw, occurrences_1km, occurrences_train,
                            ...
                            cbi.train = BI_RF$cor,
                            cbi.val = BI_RF_val$cor,
                            auc.train = AUC_rf,
                            auc.val = AUC_rf_val,
                            thresholds, threshold_used)
    rownames(data2save) <- t
    
    info_models_RF <- rbind(info_models_RF, data2save)
    write.csv(info_models_RF, "info_models_RF_DS_all.csv", row.names = FALSE)
    write.csv(info_models_RF, paste0(dir2save_rf, "info_models_RF_DS_all.csv"), row.names = FALSE)
    #info_models_RF <- fread("info_models_RF_DS_all.csv")
    #info_models_RF <- info_models_RF[-3, ]
...
info_models_RF_DS_all <- fread("/eos/jeodpp/home/users/rotllxa/lucas_grassland_data/info_models_RF_DS_all.csv", header = TRUE)
#info_models_RF_DS_all <- read.csv(unz("/eos/jeodpp/home/users/rotllxa/lucas_grassland_data/MaxEnt_02_CompTests.zip", 
#                                       "info_models_RF_DS_all"), 
...
data2save_ReleveValid_AUC <- fread("/eos/jeodpp/home/users/rotllxa/lucas_grassland_data/info_models_RF_DS_ReleveValid_AUC.csv", header = TRUE)
#data2save_ReleveValid_AUC <- read.csv(unz("/eos/jeodpp/home/users/rotllxa/lucas_grassland_data/MaxEnt_02_CompTests.zip", 
#                                          "info_models_RF_DS_ReleveValid_AUC.csv"), 
#                                      header = TRUE, sep = ",") %>% data.table
data2save_ReleveValid_AUC
...
data2save_ReleveValid <- fread("/eos/jeodpp/home/users/rotllxa/lucas_grassland_data/info_models_RF_DS_ReleveValid.csv", header = TRUE)
...
if(updated_RF == "Yes"){
  dta2plot_gbif <- info_models_RF_DS_all[, .SD, .SDcols = c("species", "auc.train", "auc.val", "cbi.train", "cbi.val")] %>%
    group_by(species) %>%
    summarise(n = n(), 
...
dta2plot_gbif_RF_DS <- info_models_RF_DS_all[, .SD, .SDcols = c("algorithm", "species", "auc.train", "auc.val", "cbi.train", "cbi.val")] %>%
  group_by(species) %>%
  summarise(n = n(), 
            GBIF_AUC.train_mean = mean(auc.train), 
            GBIF_AUC.train_SD = sd(auc.train), 
...
dta2plot_gbif_RF_DS
dta2plot_gbif_RF_DS[, algorithm := "RF_DS"]


dta2plot_gbif_maxent <- info_models_maxent_all[, .SD, .SDcols = c("algorithm", "species", "auc.train", "auc.val", "cbi.train", "cbi.val")] %>%
  group_by(species) %>%
  summarise(n = n(), 
            GBIF_AUC.train_mean = mean(auc.train), 
            GBIF_AUC.train_SD = sd(auc.train), 
...
dta2plot_gbif_maxent
dta2plot_gbif_maxent[, algorithm := "MaxEnt"]



dta2plot_gbif <- rbind(dta2plot_gbif_RF_DS, dta2plot_gbif_maxent)
dta2plot_gbif
...
dta2plot_lucas_RF_DS <- info_models_RF_DS_ReleveValid_AUC[, .SD, .SDcols = c("species", "AUC_ROC", "CBI")]  %>%
  group_by(species) %>%
  summarise(n = n(), 
            LUCAS_AUC.val_mean = mean(AUC_ROC), 
            LUCAS_AUC.val_SD = sd(AUC_ROC), 
...
dta2plot_lucas_maxent <- info_models_maxent_ReleveValid_AUC[, .SD, .SDcols = c("species", "AUC_ROC", "CBI")]  %>%
  group_by(species) %>%
  summarise(n = n(), 
            LUCAS_AUC.val_mean = mean(AUC_ROC), 
            LUCAS_AUC.val_SD = sd(AUC_ROC), 
...
dta2plot <- merge(dta2plot_gbif, dta2plot_lucas, by = c("species", "algorithm")) %>%
  select(-c("n.x", "n.y")) %>%
  pivot_longer(!c(species, algorithm), 
               names_pattern = "(.*)(_mean|_SD)$",
               names_to = c("Metrics", "Statistic")) %>%
  pivot_wider(names_from = Statistic, 
              values_from = value) %>%
...
plot1 <- ggplot(dta2plot_1, aes(LUCAS_AUC.val, GBIF_AUC.val, colour = factor(algorithm))) +
  geom_point() +
  xlim(0, 1) +
  ylim(0, 1) +
...
plot2 <- ggplot(dta2plot_2, aes(LUCAS_CBI.val, GBIF_cbi.val, colour = factor(algorithm))) +
  geom_point() +
  xlim(0, 1) +
  ylim(0, 1) +
  #geom_smooth(method = lm, se = FALSE) 
...
dta2plot_lucas_RF_DS <- info_models_RF_DS_ReleveValid[, .SD, .SDcols = c("species", "threshold", "TSS")]  %>%
  group_by(species, threshold) %>%
  summarise(n = n(), 
            LUCAS_TSS_mean = mean(TSS), 
            LUCAS_TSS_SD = sd(TSS), 
...
dta2plot_lucas_maxent <- info_models_maxent_ReleveValid[, .SD, .SDcols = c("species", "threshold", "TSS")]  %>%
  group_by(species, threshold) %>%
  summarise(n = n(), 
            LUCAS_TSS_mean = mean(TSS), 
            LUCAS_TSS_SD = sd(TSS), 
...
select(dta2plot_lucas, c(algorithm, species, threshold, LUCAS_TSS_mean))
...
plot_3 <-  ggplot(dta2plot_3, 
                  aes(LUCAS_TSS_mean, GBIF_AUC.val, colour = factor(algorithm))) +
  geom_point() +
  xlim(0, 1) +
  ylim(0, 1) +
  geom_abline(slope = 1, intercept = 0) +
